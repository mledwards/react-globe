...
export default class Globe {
  ...
  defocus() {
    const shouldDefocus =
      !this.isLocked && this.previousFocus && this.options.enableDefocus;
    const defaultBehaviour = this.callbacks.onDefocus(
      this.previousFocus,
      shouldDefocus,
    );
    if (shouldDefocus && (defaultBehaviour || defaultBehaviour === undefined)) {
  ...
  updateMarkers(markers = []) {
    ...
    const callbacks = {
      onClickMarker: (marker, markerObject, event) => {
        const defaultBehaviour = this.callbacks.onClickMarker(
          marker,
          markerObject,
          event,
        );
        if (defaultBehaviour || defaultBehaviour === undefined) {
          this.updateFocus(marker.coordinates);
        }
      },
      onTouchMarker: (marker, markerObject, event) => {
        const defaultBehaviour = this.callbacks.onTouchMarker(
          marker,
          markerObject,
          event,
        );
      ...
      onMouseOutMarker: (marker, markerObject, event) => {
        const defaultBehaviour = this.callbacks.onMouseOutMarker(
          marker,
          markerObject,
          event,
        );
      ...
      onMouseOverMarker: (marker, markerObject, event) => {
        const defaultBehaviour = this.callbacks.onMouseOverMarker(
          marker,
          markerObject,
          event,
        );
