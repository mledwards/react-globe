...
export default class Globe {
  ...
  defocus() {
    if (!this.isLocked && this.previousFocus && this.options.enableDefocus) {
      ...
      const parameters = { tooltip: this.tooltip, options: this.options };
      this.callbacks.onDefocus(this.previousFocus, parameters);
    }
  }
  ...
  updateMarkers(markers = []) {
    ...
    const callbacks = {
      onClickMarker: (marker, markerObject, event) => {
        ...
        const parameters = { tooltip: this.tooltip, options: this.options };
        this.callbacks.onClickMarker(marker, markerObject, event, parameters);
      },
      onTouchMarker: (marker, markerObject, event) => {
        this.updateFocus(marker.coordinates);
        const parameters = { tooltip: this.tooltip, options: this.options };
        this.callbacks.onTouchMarker(marker, markerObject, event, parameters);
      },
      ...
      onMouseOverMarker: (marker, markerObject, event) => {
        const parameters = { tooltip: this.tooltip, options: this.options };
        this.callbacks.onMouseOverMarker(
          marker,
          markerObject,
          event,
          parameters,
