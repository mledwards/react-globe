...
export const defaultCallbacks = {
  onClickMarker: (marker, _markerObject, _event, parameters) => {
    parameters.updateFocus(marker.coordinates);
  },
  onTouchMarker: (marker, _markerObject, _event, parameters) => {
    parameters.updateFocus(marker.coordinates);
  },
  onDefocus: (_previousFocus, parameters) => {
    parameters.updateFocus(null);
  },
  onGlobeBackgroundTextureLoaded: () => {},
  onGlobeCloudsTextureLoaded: () => {},
  onGlobeTextureLoaded: () => {},
  onMouseOutMarker: (_marker, _markerObject, _event, parameters) => {
    parameters.tooltip.hide();
  },
  onMouseOverMarker: (_marker, markerObject, event, parameters) => {
    if (parameters.options.enableMarkerTooltip) {
      parameters.tooltip.show(
        event.clientX,
        event.clientY,
        parameters.options.markerTooltipRenderer(markerObject.marker),
      );
    }
  },
};
